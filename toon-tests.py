...
class ImreadTestCase(unittest.TestCase):
    ...
    def testOutput(self):
        ...
        self.assertTrue(0 <= min_val and max_val <= 1,
                        'Values not between 0 and 1')
...
class ConvolutionTestCase(unittest.TestCase):
    ...
    def test2D(self):
        ...
        output_gt = convolve(self.random_img_2d,
                             self.random_filt, mode='constant')
        # print("2d output_gt: \n", output_gt[95:, 95:])
        self.assertTrue(np.allclose(output, output_gt), 'Incorrect values 2d')

    def test3D(self):
        output = student.convolve(self.random_img_3d, self.random_filt)
        # print('3d output:, \n', output[95:, 95:, 0])
        output_gt = convolve(self.random_img_3d,
                             self.random_filt[:, :, None], mode='constant')
        # print('3d output_gt:, \n', output_gt[95:, 95:, 0])
        # print('did we pass third ', np.allclose(output, output_gt))

        self.assertTrue(np.allclose(output, output_gt), 'Incorrect values 3d')

    def testValuesFlipped(self):
        output = student.convolve(self.random_filt, self.random_img_2d)
        output_gt = convolve(
            self.random_filt, self.random_img_2d, mode='constant')
        # print('did we pass fourth ', np.allclose(output, output_gt))
        self.assertTrue(np.allclose(output, output_gt),
                        'Incorrect values when performing convolution with a filter larger than the image')
...
class GaussianTestCase(unittest.TestCase):
    ...
    def testValues(self):
        ...
        # print('out is: \n', out[0:3, 0:3])
        out_gt = gaussian_filter(
            self.random_img, sigma=1, truncate=2, mode='constant')
        # print('out_gt is: \n', out_gt[0:3, 0:3])
...
class GradientTestCase(unittest.TestCase):
    ...
    def testValues(self):
        ...
        test_mag = np.array(
            [0.46888004, 0.22153629, 0.14217979, 0., 0.0605548])
        # print("ours: \n", dmag)
        # print("theirs: \n", test_mag)
        self.assertTrue(np.allclose(dmag, test_mag, rtol=1e-5),
                        'Gradient magnitude does not match')

    def testValues2(self):
        mag, ori = student.gradient(self.img)
        dori = np.diagonal(ori)[[0, 69, 275, 420, 425]]
        test_ori = np.array(
            [0.78539816,  3.14159265, -2.35619449,  0., 0.78539816])
        self.assertTrue(np.allclose(dori, test_ori, rtol=1e-5),
                        'Gradient orientation does not match')
...
class Check_distance_from_lineTestCase(unittest.TestCase):
    ...
    def testValues(self):
        out = student.check_distance_from_line(
            self.x, self.y, self.theta, self.c, thresh=0.3)
        # print('our is\n', out)
        self.assertTrue(np.all(out[:3] == True) and np.all(
            out[3:] == False), 'Check distance from line incorrect.')


class Draw_linesTestCase(unittest.TestCase):
    def setUp(self):
        self.img = student.imread('test.png')
        self.lines = [[-1.57079633, 274.89321881],
                      [-1.02101761, -11.10678119], [3.14159265,  68.89321881]]

    def testValues(self):
        out = student.draw_lines(self.img, self.lines, thresh=0.5)
        red_idx = list(
            zip(*np.where(np.all(out == np.array([1, 0, 0]), axis=-1))))
        self.assertTrue((275, 100) in red_idx and (
            275, 300) in red_idx, 'Missing red line.')

    def testValues2(self):
        out = student.draw_lines(self.img, self.lines, thresh=0.5)
        red_idx = list(
            zip(*np.where(np.all(out == np.array([1, 0, 0]), axis=-1))))
        self.assertTrue((51, 104) in red_idx and (156, 276)
                        in red_idx, 'Missing red line.')

    def testValues3(self):
        out = student.draw_lines(self.img, self.lines, thresh=0.5)
        red_idx = list(
            zip(*np.where(np.all(out == np.array([1, 0, 0]), axis=-1))))
        self.assertTrue((100, 69) in red_idx and (300, 69)
                        in red_idx, 'Missing red line.')
...
class Hough_votingandLocalmaxTestCase(unittest.TestCase):
    ...
    def testHough(self):
        ...
        sol_idx = [(21, 491), (21, 603), (41, 141), (41, 353), (61, 141),
                   (61, 353), (81, 388), (81, 389), (81, 491), (81, 603)]
    ...
    def testHough2(self):
        ...
        m = matches(idx, sol_idx, 0)
        self.assertTrue(
            len(idx) < 20, 'Too many large cell values (>100 votes).')

    def testLocalmax(self):
        lines = student.localmax(self.votes, self.thetas, self.cs, 20, 15)
        sol_lines = [(-1.5707963267948966, 274.89321881345245), (-1.5707963267948966, 498.89321881345245), (-1.0210176124166828, -11.10678118654755), (0.0, -425.10678118654755), (0.0, -1.1067811865475505), (1.0210176124166828, -
                                                                                                                                                                                                               215.10678118654755), (1.570796326794897, -425.10678118654755), (1.570796326794897, -1.1067811865475505), (3.1415926535897936, 68.89321881345245), (3.1415926535897936, 274.89321881345245), (3.1415926535897936, 498.89321881345245)]
        m = matches(lines, sol_lines)
        # print("lines: ", (lines))
        # print("sol_lines: ", len(sol_lines))
        self.assertTrue(m >= 7, 'At least seven lines were not found.')

    def testLocalmax2(self):
        lines = student.localmax(self.votes, self.thetas, self.cs, 20, 15)
        sol_lines = [(-1.5707963267948966, 274.89321881345245), (-1.5707963267948966, 498.89321881345245), (-1.0210176124166828, -11.10678118654755), (0.0, -425.10678118654755), (0.0, -1.1067811865475505), (1.0210176124166828, -
                                                                                                                                                                                                               215.10678118654755), (1.570796326794897, -425.10678118654755), (1.570796326794897, -1.1067811865475505), (3.1415926535897936, 68.89321881345245), (3.1415926535897936, 274.89321881345245), (3.1415926535897936, 498.89321881345245)]
        m = matches(lines, sol_lines)
